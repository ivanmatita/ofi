...
const App = () => {
  ...
  const handleSaveInvoice = async (inv: Invoice, sId: string, action?: string) => {
      ...
      // REGRA AGT: Bloqueio de certificação fora de sequência ou cronologia
      if (action === 'CERTIFY') {
          // 1. Sequencialidade: Verificar se existe rascunho anterior pendente do mesmo tipo/série
          ...
          // 2. Cronologia: Verificar se a data é anterior ao último documento certificado
      ...
      // REGRA AGT: Atribuição de numeração fiscal automática no ato da certificação
      if (action === 'CERTIFY' && !inv.isCertified) {
          ...
          // Estrutura legal: TIPO SÉRIE ANO/Nº (ex: FT A 2024/1)
          ...
          // Documento em rascunho não possui número fiscal definitivo
          finalInv.number = '---';
      }
      ...
      try {
        ...
        const syncPayload: any = {
          ...
          // Removido devido a erro de schema cache no Supabase
          // hash: finalInv.hash,
  ...
  const handleDeleteInvoice = async (id: string) => {
      ...
      // REGRA AGT: Bloqueio de eliminação de documentos certificados
      if (inv?.isCertified) {
