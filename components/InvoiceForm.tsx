...
const InvoiceForm: React.FC<InvoiceFormProps> = ({ 
  ...
  // Filial / Séries autorizadas
  const allowedSeries = useMemo(() => {
      // REGRA: Séries ativas e que permitem ao utilizador atual (ou sem restrição)
      return series.filter(s => 
          s.isActive && (s.allowedUserIds.length === 0 || (currentUserId && s.allowedUserIds.includes(currentUserId)))
      );
  ...
  useEffect(() => {
    // Definir série padrão se não houver uma selecionada
                ...
                <div className="p-5 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                         ...
                         <div className="space-y-1">
                             ...
                             <select className="w-full p-2 border border-slate-200 rounded-lg bg-slate-50 text-sm font-bold text-blue-700" value={selectedSeriesId} onChange={(e) => setSelectedSeriesId(e.target.value)} disabled={isRestricted}>
                                <option value="">Selecione Série...</option>
                                {allowedSeries.map(s => <option key={s.id} value={s.id}>{s.name} ({s.code}) - {s.type}</option>)}
                            </select>
                         </div>
                         <div className="space-y-1">
                            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Local de Trabalho / Obra (Cloud)</label>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="bg-slate-50 border-b border-slate-200 px-5 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-slate-700 text-sm uppercase">Seleção de Cliente (Base Cloud)</h3>
                    </div>
                </div>
                <div className="p-5">
