...
import { 
  ...
  CreditCard, User, Phone, List, ShoppingCart, Calculator, CheckCircle, 
...
const StockManager: React.FC<StockManagerProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'STOCK_GERAL' | 'PRODUCTS' | 'WAREHOUSES'>('DASHBOARD');
      ...
      try {
          ...
          if (prods) {
              setProducts(prods.map(p => ({ 
                ...
                stock: 0, warehouseId: '', unit: 'un', category: 'Geral', minStock: 0, priceTableId: 'pt1', barcode: p.barcode 
              })));
          }
  ...
  const allMovements = useMemo(() => {
    ...
    const purchaseMovements: StockMovement[] = purchases.flatMap(pur => 
        pur.items.filter(it => it.productId).map(it => ({
            id: `pur-${pur.id}-${it.id}`,
            date: pur.date,
            type: 'ENTRY',
            productId: it.productId!,
  ...
  const stockStats = useMemo(() => {
    const stats: Record<string, { in: number, out: number, balance: number, name: string, artNo: string, barcode?: string }> = {};
    products.forEach(p => {
        stats[p.id] = { in: 0, out: 0, balance: 0, name: p.name, artNo: p.id.substring(0,8).toUpperCase(), barcode: p.barcode };
    });
    allMovements.forEach(m => {
        if (stats[m.productId]) {
            if (m.type === 'ENTRY') stats[m.productId].in += m.quantity;
            else stats[m.productId].out += m.quantity;
        }
    });
  ...
  const handleAddItemToEntry = (prodId: string) => {
      ...
      if (prod) {
          setItems([...items, { productId: prod.id, name: prod.name, quantity: 1, notes: '' }]);
      }
  };
                  ...
                  <table className="w-full text-[10px] text-left border-collapse">
                      <thead className="bg-slate-100 text-slate-700 font-black uppercase border-b-2 border-slate-300">
                          <tr>
                              ...
                              <th className="p-3">Barcode / Série</th>
                              <th className="p-3 text-right">Total Entradas</th>
                              <th className="p-3 text-right">Total Saídas</th>
    ...
    <div className="space-y-6 animate-in fade-in pb-20">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            ...
            <div className="flex items-center gap-2 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('STOCK_GERAL')} className={`px-4 py-2 rounded-lg text-xs font-bold border transition flex items-center gap-2 ${activeTab === 'STOCK_GERAL' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50'}`}>
        ...
        {activeTab === 'PRODUCTS' && (
            <div className="bg-white border border-slate-300 rounded shadow-sm overflow-hidden animate-in zoom-in-95">
                <div className="p-4 flex justify-between items-center bg-slate-100 border-b border-slate-200">
                    <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Artigos Sincronizados na Cloud</h2>
                    <button onClick={() => { setWarehouseId(''); setItems([]); setShowProductModal(true); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-xs font-bold hover:bg-blue-700 shadow-md">
                        <Plus size={16}/> Entrada de Stock
                    </button>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-[11px] text-left">
                        <thead className="bg-slate-700 text-white font-bold uppercase">
                            <tr><th className="p-3">Artigo</th><th className="p-3 text-right">Último Custo</th><th className="p-3 text-center">Estado Cloud</th></tr>
                        </thead>
                        <tbody className="divide-y divide-slate-200">
                            {products.map((p) => (
        ...
        {showProductModal && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl animate-in zoom-in-95 overflow-hidden">
                    <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
                        ...
                        <button onClick={() => setShowProductModal(false)} className="hover:bg-slate-800 p-2 rounded-full transition"><X size={24}/></button>
                    </div>
                        ...
                        <div className="border border-slate-200 rounded-2xl overflow-hidden">
                            <table className="w-full text-left text-xs">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    ...
                                    {items.length === 0 && (
                                        <tr><td colSpan={4} className="p-12 text-center text-slate-300 italic">Nenhum item adicionado à lista.</td></tr>
                                    )}
                                </tbody>
                            </table>
                        </div>
